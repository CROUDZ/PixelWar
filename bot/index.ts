...
// Initialiser le logger avec le client Discord
initializeLogger(client);

// Monter la route logger
...
process.on("unhandledRejection", (reason) => {
  ...
  logToDiscord(`Rejection non gérée: ${reason}`, "error");
});

// Gérer l'arrêt gracieux pour flush les logs
process.on("SIGINT", async () => {
  console.log("Arrêt en cours, flush des logs...");
  await flushLogs();
  process.exit(0);
});

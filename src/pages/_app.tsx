...
useEffect(() => {
  ...
  if (typeof window !== 'undefined' && !(window as { captureInterval?: NodeJS.Timeout }).captureInterval) {
    const capture = async () => {
      try {
    ...
    (window as { captureInterval?: NodeJS.Timeout }).captureInterval = interval;

    return () => {
      if ((window as { captureInterval?: NodeJS.Timeout }).captureInterval) {
        clearInterval((window as { captureInterval?: NodeJS.Timeout }).captureInterval);
        delete (window as { captureInterval?: NodeJS.Timeout }).captureInterval;
      }
    };
  }
}, []);
